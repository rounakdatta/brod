...
-export([start/0, stop/0, squery/2, equery/3, xquery/3, process/1]).
-export([start/2, stop/1]).
-export([init/1]).

-record(kafka_message, {offset, magic_byte, attributes, key = <<"">>, value = <<"">>, crc, junk1, junk2}).
...
process(#kafka_message{offset = OFFSET, magic_byte = MB, attributes = ATTR, key = KEY, value = VAL, crc = CRC, junk1 = JNK1, junk2 = JNK2}) ->
        io:format("poolboy received : ~p ~p ~p ~p ~p ~p ~p ~p~n", [OFFSET, MB, ATTR, KEY, VAL, CRC, JNK1, JNK2]).
